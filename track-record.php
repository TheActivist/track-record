...
//  Check if current cache file exists.

// include('top-cache.php');

//  Start function to create new table row populated with current data.
...
//Declare variables
	...
	$symbol = $a['symbol'];
	$date = $a['date'];
	$av_url = str_replace("XXXX", $symbol, "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=XXXX&outputsize=full&apikey=KSSEAXNHAHCG6SD5");
	$json = file_get_contents($av_url);
	...
	// Find lowest price since publication date
    ...
    // Compute percentage change from price at publication to lowest price since publication
...
// Cache the content to a cache file.
